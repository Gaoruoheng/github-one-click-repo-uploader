...
$RemoteName = if ($Config.remoteName) { $Config.remoteName } else { "origin" }
$BranchName = if ($Config.branchName) { $Config.branchName } else { "main" }
$RepositoryUrl = if ($Config.repositoryUrl) { $Config.repositoryUrl } else { "" }
...
function Assert-ProjectReady {
  if (-not (Test-Path $ProjectRoot)) {
    throw "项目文件夹不存在：$ProjectRoot"
  }
  if (-not (Test-Path (Join-Path $ProjectRoot ".git"))) {
    throw "项目文件夹不是 Git 仓库：$ProjectRoot"
  }
}
